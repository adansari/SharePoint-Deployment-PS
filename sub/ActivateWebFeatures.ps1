...
Write-Host "Activating Web features on '$url'" -ForegroundColor Green 
...
{	
	...
	{
		# Feature not found
		Write-Host "  * No Web scoped feature named '$feature_name' was found" -ForegroundColor Red 
		continue 
	}
	...
	# Check if feature is already activated in the Site scope
    	$private:enabledfeature = Get-SPFeature -Web $url -identity $feature_id -ErrorAction:SilentlyContinue
